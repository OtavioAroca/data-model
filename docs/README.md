...
O script:
- Lê `catalogo/index.yaml` (15 subdomínios)
- Lê cada `catalogo/<slug>/catalogo.yaml` (produtos por subdomínio)
- Lê `catalogo/compras-autorizacao/mapeamento-tecnico.yaml` (mapeamento técnico)
- Lê `catalogo/compras-autorizacao/schema-fisico/schema-fisico.yaml` (29 tabelas + colunas)
- Calcula lineage, relacionamentos inferidos e detecção de gaps
- Injeta tudo em `docs/apresentacao.template.html`
- Escreve `docs/apresentacao.html` (arquivo final)
...
1. Abra `docs/apresentacao.html` no navegador
