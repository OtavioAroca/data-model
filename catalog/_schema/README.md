...
- **`catalog.schema.json`** — Contrato de `catalog/<slug>/catalog.yaml` (produtos de dados de um subdomínio)
- **`mapeamento-tecnico.schema.json`** — Contrato de `catalog/<slug>/mapeamento-tecnico.yaml` (mapeamento para tabelas físicas)
- **`index.schema.json`** — Contrato de `catalog/index.yaml` (índice dos 15 subdomínios)
...
```bash
#!/bin/bash
python3 scripts/validate_catalog.py || exit 1
```
...
Todo subdomínio (`catalog/<slug>/`) tem uma estrutura padrão:

```
catalog/<slug>/
├── catalog.md              # Documentação em Markdown (gerada)
├── catalog.yaml            # Produtos de dados (Core/Integration/Analytics)
...
**Observações:**
- `catalog.md`, `catalog.yaml`: obrigatório em todo subdomínio.
...
Abra `catalog/<novo-slug>/catalog.yaml` em um editor com suporte a YAML Schema (VSCode + YAML extension):
...
Abra `catalog/index.yaml` e adicione uma entrada em `subdominios[]`:
...
```bash
cp catalog/_schema/mapeamento-tecnico.template.yaml catalog/<slug>/mapeamento-tecnico.yaml
```
...
```bash
mkdir -p catalog/<slug>/schema-fisico
```
...
```bash
cp catalog/_schema/schema-fisico.template.yaml catalog/<slug>/schema-fisico/schema-fisico.yaml
# Edite catalog/<slug>/schema-fisico/schema-fisico.yaml
...
Crie `catalog/<slug>/schema-fisico/simulate.py` com uma função `generate(n)`:
...
Referência: [`catalog/compras-autorizacao/schema-fisico/simulate.py`](../../compras-autorizacao/schema-fisico/simulate.py).
...
Cria `catalog/<slug>/schema-fisico/ddl.sql` com as `CREATE TABLE` statements (dialeto SQLite, com tipos, PK, FK, NOT NULL).
...
Cria `catalog/<slug>/schema-fisico/mock.db` com:
...
Abra `catalog.schema.json` e adicione `owner_email` à lista `required`:
...
Ele aponta: "catalog/elegibilidade/catalog.yaml: 'owner_email' is a required property".
...
O validador Python (`scripts/validate_catalog.py`) também confere:

1. **Coerência de slugs**: todo slug em `index.yaml` tem pasta correspondente em `catalog/`, e vice-versa.
...
5. **Avisos**: produtos no mapeamento técnico que não existem no `catalog.yaml` geram aviso (não erro) — é proposital para "insumo interno".
...
4. **Não altere o nome do slug após criado** — é a chave que liga o índice à pasta. Se precisar, atualize em 3 lugares: pasta, `nome` em `catalog.yaml`, e `slug` em `index.yaml`.
...
mkdir catalog/investimentos
cp catalog/_schema/catalog.template.yaml catalog/investimentos/catalog.yaml

# 2. Editar catalog/investimentos/catalog.yaml (editor valida em tempo real)
...
# 5. Commit
git add catalog/ && git commit -m "Adicionar subdomínio Investimentos com 5 produtos"
```
...
R: Git. `git log catalog/_schema/catalog.schema.json` mostra histórico, `git show <hash>` mostra o que mudou, `git revert <hash>` desfaz. Depois, rodeia validador e corrige os arquivos conforme a mudança anterior.
